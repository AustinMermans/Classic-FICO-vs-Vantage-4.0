...
def test_load_score_files_nulls_unavailable_and_out_of_range_scores(tmp_path: Path):
    source = tmp_path / "scores.txt"
    source.write_text(HEADER + "\n2020Q2|123|9999|299|850|851|700\n")
    result = load_score_files([source], COLUMNS).row(0, named=True)
    assert result["fico_10t_current_method"] is None
    assert result["fico_10t_trimerge"] is None
    assert result["fico_10t_bimerge_lowest"] == 850
    assert result["fico_10t_bimerge_median"] is None
    assert result["fico_10t_bimerge_highest"] == 700
